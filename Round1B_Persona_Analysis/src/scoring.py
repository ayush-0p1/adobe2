...
def heading_boost_fn(
    page: int, headings: Dict[int, List[str]], text: str, weight: float
) -> float:
...
def rank_chunks(
    ...
    for i, ch in enumerate(chunks):
        ...
        s = base
        s += keyword_boost(
            ch["text"], kwset, cfg["rank"]["keyword_boost"]
        )
        s += heading_boost_fn(
            ch["page"], headings, ch["text"], cfg["rank"]["heading_boost"]
        )
